...
<title>广东省文化馆-馆务公开-政策法规-${wh_zx_colinfo.clnftltle }</title>
    ...
    <div class="header-nav-bg">
        <div class="header-nav">
            ...
            <ul>
                ...
                <li class="active"><a href="${basePath }/agdgwgk/zhinan">业务指南</a></li>
...
<div class="public-crumbs">
    ...
    <span><a href="${basePath }/agdgwgk/zhinan">业务指南</a></span><span>></span>
           ...
           <c:if test="${not empty loadlist}">
            <div class="file-download-cont">
				...
				<div class="next-notice">
					<a href="${basePath }/agdgwgk/zhinaninfo?id=${wh_zx_colinfo_next.clnfid}">下一篇<span>${wh_zx_colinfo_next.clnftltle}</span></a>
				</div>
			...
			<div class="public-share">
				...
				<span class="btn weibo"><a href="javascript:void(0)" class="fxweibo" target="_blank"></a></span>            </div>
			</div>
				...
				<ul>
					<c:if test="${wh_zx_colinfo_ref != null && fn:length(wh_zx_colinfo_ref) > 0}">
						<c:forEach items="${wh_zx_colinfo_ref }" var="row" varStatus="s">
							<li><a href="${basePath }/agdgwgk/zhinaninfo?id=${row.clnfid}">${row.clnftltle }</a><p class="time"><fmt:formatDate value="${row.clnfcrttime}" pattern="yyyy-MM-dd" /></p></li>
						</c:forEach>
					</c:if>
				</ul>
			</div>
