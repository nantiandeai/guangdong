...
<title>广东省文化馆-馆务公开-业务指南</title>
<script type="text/javascript">
/** page onload event */
$(function(){
	//加载分页工具栏
	genPagging('whgPagging', '${page}' || 1, 10, '${total}', function(page, rows){
		window.location.href = '${basePath}/agdgwgk/zhinan?page='+page+'&rows='+rows;
	});
});
</script>
</head>
...
<!--资讯广告开始-->
<div class="public-information-ad" style="background:url(${basePath }/static/assets/img/museum/adBanner1.jpg) no-repeat 50% 50%">
	<div class="posi-fix">
    	<div class="titleBg" style="background:url(${basePath }/static/assets/img/museum/title-bg-1.png) no-repeat 50% 50%;"></div>
    </div>
</div>
		...
		<c:choose>
			<c:when test="${rows != null && fn:length(rows) > 0}">
				<c:forEach items="${rows }" var="row" varStatus="s">
			    	<li>
			        	<i></i>
			        	<h2><a href="${basePath }/agdgwgk/zhinaninfo?id=${row.clnfid}">${row.clnftltle }</a></h2>
