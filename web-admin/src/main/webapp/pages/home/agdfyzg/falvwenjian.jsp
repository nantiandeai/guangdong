...
<title>广东省文化馆-非遗展馆-法律文件列表</title>
...
$(function(){
	...
	genPagging('whgPagging', '${page}' || 1, 10, '${total}', function(page, rows){
		window.location.href = '${basePath}/agdfyzg/falvwenjian?page='+page+'&rows='+rows;
	});
});
</script>
</head>
	...
	<div class="header-nav-bg">
		<div class="header-nav">
			...
			<ul>
				...
				<li class="active"><a href="${basePath }/agdfyzg/falvwenjian">法律文件</a></li>
...
<%--<div class="main-info-bg main-info-no-padding main-info-bgColorW">--%>
	<%--<div class="main-info-container">--%>
					...
					<%--<c:forEach items="${rows }" var="row" varStatus="s">--%>
					    <%--<li>--%>
			                ...
			                <%--<div class="notice-list-info">--%>
			                	<%--<div class="arrow"></div>--%>
			                    <%--<div class="info">--%>
			                    	<%--<h2><a href="${basePath }/agdfyzg/falvwenjianinfo?id=${row.clnfid}">${row.clnftltle }</a></h2>--%>
			                        <%--<p>${row.clnfintroduce }</p>--%>
									<%--<span class="more"><a href="${basePath }/agdfyzg/falvwenjianinfo?id=${row.clnfid}">查看详情</a></span>--%>
			                    <%--</div>--%>
			                <%--</div>--%>
				        <%--</li>--%>
					<%--</c:forEach>--%>
        ...
        <%----%>
        	<%--<!-- 分页 -->--%>
			<%--&lt;%&ndash;<div class="green-black" id="whgPagging"></div>&ndash;%&gt;--%>
			<%--<!-- 分页-END -->--%>
		<%--</div>--%>
	<%--</div>--%>
<%--</div>--%>
		...
		<c:choose>
			<c:when test="${rows != null && fn:length(rows) > 0}">
				<c:forEach items="${rows }" var="row" varStatus="s">
					<li>
						<i></i>
						<h2><a href="${basePath }/agdfyzg/falvwenjianinfo?id=${row.clnfid}">${row.clnftltle }</a><span class="time"><fmt:formatDate value="${row.clnfcrttime}" pattern="yyyy-MM-dd" /></span></h2>
						<%--<p>${row.clnfintroduce }</p>--%>
					</li>
				</c:forEach>
			</c:when>
