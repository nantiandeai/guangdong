...
<title>广东省文化馆-非遗中心-申报指南列表</title>
...
$(function(){
	...
	genPagging('whgPagging', '${page}' || 1, 10, '${total}', function(page, rows){
		window.location.href = '${basePath}/agdfyzg/shenbao?page='+page+'&rows='+rows;
	});
});
</script>
</head>
	...
	<div class="header-nav-bg">
		<div class="header-nav">
			...
			<ul>
				...
				<li class="active last"><a href="${basePath }/agdfyzg/shenbao">申报指南</a></li>
					...
					<%--<c:when test="${rows != null && fn:length(rows) > 0}">--%>
						<%--<c:forEach items="${rows }" var="row" varStatus="s">--%>
			                ...
			                <%--<div class="notice-list-info">--%>
			                	<%--<div class="arrow"></div>--%>
			                    <%--<div class="info">--%>
			                    	<%--<h2><a href="${basePath }/agdfyzg/shenbaoinfo?id=${row.clnfid}">${row.clnftltle }</a></h2>--%>
			                        <%--&lt;%&ndash;<p>${row.clnfintroduce }</p>&ndash;%&gt;--%>
									<%--<span class="more"><a href="${basePath }/agdfyzg/shenbaoinfo?id=${row.clnfid}">查看详情</a></span>--%>
			                    <%--</div>--%>
			                <%--</div>--%>
			            <%--</li>--%>
						<%--</c:forEach>--%>
		...
		<c:choose>
			<c:when test="${rows != null && fn:length(rows) > 0}">
				<c:forEach items="${rows }" var="row" varStatus="s">
					<li>
						<i></i>
						<h2><a href="${basePath }/agdfyzg/shenbaoinfo?id=${row.clnfid}">${row.clnftltle }</a><span class="time"><fmt:formatDate value="${row.clnfcrttime}" pattern="yyyy-MM-dd" /></span></h2>
