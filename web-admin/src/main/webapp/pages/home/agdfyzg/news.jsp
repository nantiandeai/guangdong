...
$(function(){
	...
	genPagging('whgPagging', '${page}' || 1, 10, '${total}', function(page, rows){
		window.location.href = '${basePath}/agdfyzg/news?page='+page+'&rows='+rows;
	});
});
</script>
</head>
					...
					<c:forEach items="${rows }" var="row" varStatus="s">
					    <li>
			                ...
			                <div class="notice-list-info">
			                	<div class="arrow"></div>
			                    <div class="info">
			                    	<h2><a href="${basePath }/agdfyzg/newsinfo?id=${row.clnfid}">${row.clnftltle }</a></h2>
			                        <%--<p>${row.clnfintroduce }</p>--%>
									<c:choose>
										<c:when test="${fn:length(row.clnfintroduce) > 145}">
											<p class="i">${fn:substring(row.clnfintroduce, 1, 145)}......</p>
										</c:when>
										<c:otherwise>
											<p class="i">${row.clnfintroduce}</p>
										</c:otherwise>
									</c:choose>
									<span class="more"><a href="${basePath }/agdfyzg/newsinfo?id=${row.clnfid}">查看详情</a></span>
			                    </div>
			                </div>
			            </li>
					</c:forEach>
