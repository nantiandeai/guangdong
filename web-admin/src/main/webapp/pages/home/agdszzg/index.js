...
function loadArtContent(page, rows){
	...
	var _url = getFullUrl('./agdszzg/srchszzg?p=1');
...
function showList(dataRows){
	var _html = '';
	if (!dataRows.length || dataRows.length < 1){
	...
	for(var i=0; i<dataRows.length; i++){
		...
		_html += '<div class="img"> <a href="'+getFullUrl('agdszzg/zglist?id='+dataRows[i].exhid)+'"><img src="'+(imgServerAddr+dataRows[i].exhpic)+'" width="300" height="190"></a> </div>';
		_html += '<div class="info">';
		_html += '<h2><a href="'+getFullUrl('agdszzg/zglist?id='+dataRows[i].exhid)+'">'+dataRows[i].exhtitle+'</a></h2>';
		_html += '<p>时间 :<span>'+dateFMT(dataRows[i].exhcrttime)+'</span></p>';
