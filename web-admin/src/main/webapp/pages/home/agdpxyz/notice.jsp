...
$(function(){
	...
	genPagging('whgPagging', '${page}' || 1, 10, '${total}', function(page, rows){
		window.location.href = '${basePath}/agdpxyz/notice?page='+page+'&rows='+rows;
	});
});
</script>
</head>
						...
						<c:forEach items="${rows }" var="row" varStatus="s">
						     <li>
								...
								<div class="notice-list-info">
									<div class="arrow"></div>
									<div class="info">
										<h2><a href="${basePath }/agdpxyz/noticeinfo?id=${row.clnfid}">${row.clnftltle }</a></h2>
										<p>${row.clnfintroduce }</p>
										<span class="more"><a href="${basePath }/agdpxyz/noticeinfo?id=${row.clnfid}">查看详情</a></span>
									</div>
								</div>
							</li>
						</c:forEach>
