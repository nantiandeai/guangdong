...
<title>广东省文化馆-资讯动态-工作动态-${wh_zx_colinfo.clnftltle }</title>
    ...
    <div class="header-nav-bg">
    	<div class="header-nav">
            ...
            <ul>
                ...
                <li class="active"><a href="${basePath }/agdzxdt/working/">工作动态</a></li>
...
<div class="public-crumbs">
    ...
    <span><a href="${basePath }/agdzxdt/index">资讯动态</a></span><span>></span>
    <span><a href="${basePath }/agdzxdt/working">工作动态</a></span><span>></span>
		...
		<div class="public-left-main">
			<div class="noticeInfo-main">
				...
				<div class="next-notice">
					<a href="${basePath }/agdzxdt/workinginfo?id=${wh_zx_colinfo_next.clnfid}">下一篇<span>${wh_zx_colinfo_next.clnftltle}</span></a>
				</div>
				...
				<ul>
					<c:if test="${wh_zx_colinfo_ref != null && fn:length(wh_zx_colinfo_ref) > 0}">
						<c:forEach items="${wh_zx_colinfo_ref }" var="row" varStatus="s">
							<li><a href="${basePath }/agdzxdt/workinginfo?id=${row.clnfid}">${row.clnftltle }</a><p class="time"><fmt:formatDate value="${row.clnfcrttime}" pattern="yyyy-MM-dd" /></p></li>
						</c:forEach>
					</c:if>
					<div class="public-no-message ${not empty wh_zx_colinfo_ref? 'none':''}"></div>
				</ul>
			</div>
		</div>
		<!-- 相关推荐-END -->
