...
<title>广东省文化馆-资讯动态-热点聚集-${wh_zx_colinfo.clnftltle }</title>
...
<div class="public-crumbs">
    ...
    <span><a href="${basePath }/agdzxdt/hot">热点聚集</a></span><span>></span>
		...
		<div class="public-left-main">
			<div class="noticeInfo-main">
				...
				<div class="next-notice">
					<a href="${basePath }/agdzxdt/hotinfo?id=${wh_zx_colinfo_next.clnfid}">下一篇<span>${wh_zx_colinfo_next.clnftltle}</span></a>
				</div>
				...
				<ul>
					<c:if test="${wh_zx_colinfo_ref != null && fn:length(wh_zx_colinfo_ref) > 0}">
						<c:forEach items="${wh_zx_colinfo_ref }" var="row" varStatus="s">
							<li><a href="${basePath }/agdzxdt/hotinfo?id=${row.clnfid}">${row.clnftltle }</a><p class="time"><fmt:formatDate value="${row.clnfcrttime}" pattern="yyyy-MM-dd" /></p></li>
						</c:forEach>
					</c:if>
