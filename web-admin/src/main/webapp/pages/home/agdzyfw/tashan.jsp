...
$(function(){
	...
	//加载分页工具栏
	genPagging('whgPagging', _page || 1, _pageSize || 9, _total, function(page, rows){
		window.location.href = '${basePath}/agdzyfw/tashan?page='+page+'&rows='+rows;
	});
});
</script>
</head>
					...
					<c:forEach items="${rows }" var="row" varStatus="s">
					    <li>
			                ...
			                <div class="notice-list-info">
			                	<div class="arrow"></div>
			                    <div class="info">
			                    	<h2><a href="${basePath }/agdzyfw/tashaninfo?id=${row.clnfid}">${row.clnftltle }</a></h2>
			                        <p>${row.clnfintroduce }</p>
									<span class="more"><a href="${basePath }/agdzyfw/tashaninfo?id=${row.clnfid}">查看详情</a></span>
			                    </div>
			                </div>
			            </li>
					</c:forEach>
