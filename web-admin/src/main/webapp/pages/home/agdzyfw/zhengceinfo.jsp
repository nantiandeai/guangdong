...
<title>广东省文化馆-志愿服务-政策法规-${wh_zx_colinfo.clnftltle }</title>
...
<div class="public-crumbs">
    ...
    <span><a href="${basePath }/agdzyfw/zhengce">政策法规</a></span><span>></span>
		...
		<div class="public-left-main">
			<div class="noticeInfo-main">
				...
				<div class="next-notice">
					<a href="${basePath }/agdzyfw/zhengceinfo?id=${wh_zx_colinfo_next.clnfid}">下一篇<span>${wh_zx_colinfo_next.clnftltle}</span></a>
				</div>
				<!-- 分享 -->
				...
				<ul>
					<c:if test="${wh_zx_colinfo_ref != null && fn:length(wh_zx_colinfo_ref) > 0}">
						<c:forEach items="${wh_zx_colinfo_ref }" var="row" varStatus="s">
							<li><a href="${basePath }/agdzyfw/zhengceinfo?id=${row.clnfid}">${row.clnftltle }</a><p class="time"><fmt:formatDate value="${row.clnfcrttime}" pattern="yyyy-MM-dd" /></p></li>
						</c:forEach>
					</c:if>
				</ul>
			</div>
