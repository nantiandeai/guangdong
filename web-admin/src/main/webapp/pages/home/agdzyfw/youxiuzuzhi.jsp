...
<title>广东省文化馆-志愿服务-风采展示-优秀组织</title>
...
$(function(){
	...
	genPagging('whgPagging', _page, _pageSize, _total, function(page, rows){
		window.location.href = '${basePath}/agdzyfw/youxiuzuzhi?page='+page+'&rows='+rows;
	});
})
</script>
</head>
    ...
    <div class="categoryChange">
        <div class="row clearfix">
            ...
            <div class="adrList">
                ...
                <span class="item active"><a href="${basePath }/agdzyfw/youxiuzuzhi">优秀组织</a></span>
            </div>
        </div>
    </div>
            	...
            	<c:choose>
				   <c:when test="${not empty rows }">  
					   <c:forEach items="${rows }" var="row" varStatus="s">
		                <li>
		                    <a href="${basePath }/agdzyfw/youxiuzuzhiinfo?zyfczzid=${row.zyfczzid}">
		                        <div class="img">
		                        	<img src="${imgServerAddr}/${row.zyfczzpic}" width="380" height="240"/>
		                            <div class="mask"></div>
		                        </div>
		                    </a>
		                    <div class="detail">
		                        <h2>${row.zyfczzshorttitle}</h2>
		                        <p>服务地区：<span>${row.zyfczzscope}</span></p>
		                    </div>
		                </li>
	                </c:forEach>     
				   </c:when>
