...
$(function(){
	...
	genPagging('whgPagging', '${page}' || 1, 10, '${total}', function(page, rows){
		window.location.href = '${basePath}/agdzyfw/news?page='+page+'&rows='+rows;
	});
});
</script>
</head>
					...
					<c:when test="${rows != null && fn:length(rows) > 0}">
						<c:forEach items="${rows }" var="row" varStatus="s">
			                ...
			                <div class="notice-list-info">
			                	<div class="arrow"></div>
			                    <div class="info">
			                    	<h2><a href="${basePath }/agdzyfw/newsinfo?id=${row.clnfid}">${row.clnftltle }</a></h2>
			                        <p>${row.clnfintroduce }</p>
									<span class="more"><a href="${basePath }/agdzyfw/newsinfo?id=${row.clnfid}">查看详情</a></span>
			                    </div>
			                </div>
			            </li>
						</c:forEach>
