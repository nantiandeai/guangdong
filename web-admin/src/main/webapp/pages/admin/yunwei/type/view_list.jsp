...
<head>
    ...
    <c:choose>
        ...
        <c:when test="${classify == 6}">
            <c:set var="pageTitle" value="区域配置"></c:set>
        </c:when>
        ...
        <c:when test="${classify == 13}">
            <c:set var="pageTitle" value="志愿活动类型配置"></c:set>
        </c:when>
        <c:when test="${classify == 14}">
            <c:set var="pageTitle" value="资源分类配置"></c:set>
        </c:when>
        <c:otherwise>
...
<table id="whgdg" title="${pageTitle}" class="easyui-datagrid" style="display: none"
       data-options="fit:true, striped:true, rownumbers:true, fitColumns:true, singleSelect:true, checkOnSelect:true, selectOnCheck:true, pagination:true, toolbar:'#whgdg-tb', url:'${basePath}/admin/yunwei/type/srchList4p?type=${classify}'">
    <thead>
    <tr>
